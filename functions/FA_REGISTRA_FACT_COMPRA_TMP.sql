...
CREATE OR REPLACE FUNCTION FA_REGISTRA_FACT_COMPRA_TMP(
    ...
    IF v_funcion = 'OK' THEN
        ...
        IF v_valajus BETWEEN (v_parajus*-1) AND v_parajus OR v_valajus=0.0  THEN 
            ...
            INSERT INTO FA_TFCPRD (FCPRD_FACOM,FCPRD_DSKA,FCPRD_CANT,FCPRD_SUBT,FCPRD_IVA,FCPRD_PIVA,FCPRD_TOTA,FCPRD_ESTA,FCPRD_FECH,fcprd_inve,fcprd_cantinve)
            SELECT v_consec,FCPRD_TMP_DSKA,FCPRD_TMP_CANT,FCPRD_TMP_SUBT,FCPRD_TMP_IVA,FCPRD_TMP_PIVA,FCPRD_TMP_TOTA,'A',NOW(),fcprd_tmp_inve,fcprd_tmp_cantinve FROM FA_TFCPRD_TMP
